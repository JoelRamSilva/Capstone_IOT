...
//  Serial.print(F("\tPM10  AQI China: ")); Serial.println(data.aqi_pm100_china);
 ...
 int i;
  for (int i = 0; i < 10; i++) {
    PM25_AQI_Data data;
    if (!aqi.read(&data)) { Serial.println("Could not read from AQI"); delay(200); continue; }

    float p03_to_25 = (float)data.particles_03um - (float)data.particles_25um;

    Serial.print("PC_0.3_2.5: ");
    Serial.println(p03_to_25);

    getAvr(p03_to_25, 6); // this already waits ~1s per call
  }
}